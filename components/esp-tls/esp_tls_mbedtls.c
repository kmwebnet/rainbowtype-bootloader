...
static esp_err_t set_pki_context(esp_tls_t *tls, const esp_tls_pki_t *pki)
{
    ...
    if (0 != atcab_init(&atcacfg))
	{
	    printf("Failed to init ECC608\n");
        return ESP_FAIL;    
    }

    /* Convert to an mbedtls key */
    if (0 != atca_mbedtls_pk_init(pki->pk_key, (uint16_t)keynum))
    {
    ...
    if (0 != atca_mbedtls_cert_add(pki->public_cert, &g_cert_def_2_device))
    {
	...
	if (ATCA_SUCCESS != (status = atcab_init(&atcacfg)))
	{
    ...
    if (0 != atca_mbedtls_cert_add(pki->public_cert, &g_cert_def_1_signer))
    {
        printf("Failed to parse Signer CA cert from device\n");
        return ESP_FAIL;
    }
    ...
    mbedtls_ssl_conf_curves(&tls->conf, curve);
