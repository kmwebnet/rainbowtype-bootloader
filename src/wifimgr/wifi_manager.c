...
extern char ssidresult[MAX_SSID_SIZE];
extern char passresult[MAX_PASSWORD_SIZE];
		...
		if(uxBits & WIFI_MANAGER_REQUEST_STA_CONNECT_BIT){
			...
			if(uxBits & (WIFI_MANAGER_WIFI_CONNECTED_BIT | WIFI_MANAGER_STA_DISCONNECT_BIT)){
				...
				if(wifi_manager_lock_json_buffer( portMAX_DELAY )){
					...
					memcpy(ssidresult, wifi_manager_config_sta->sta.ssid, strlen((const char *)wifi_manager_config_sta->sta.ssid));
					memcpy(passresult, wifi_manager_config_sta->sta.password, strlen((const char *)wifi_manager_config_sta->sta.password));
